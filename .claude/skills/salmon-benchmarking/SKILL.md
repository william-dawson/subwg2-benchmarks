...
| RIKYU GPU (GB200/Blackwell, `nvhpc-openacc` build, `nvhpc/26.3`), **1 rank = 1 GPU** | NVIDIA HPC-X MPI (bundled with `nvhpc/26.3` module), `mpirun -n 1` (salmon-build recipe) | Same 1×1×1 input | **16.82s** (internal `total calculation time` from the code's own timer, not wall-clock — 100/100 `nscf` iterations, converged: Total Energy -6099.94333354 eV, gap 5.717 eV — correct, matches every other machine) | **not yet measured** — only GS has been run on Rikyu's GPU build so far (both 1-GPU and the still-unresolved 4-GPU case, see salmon-build) | GS alone is the **fastest GS time recorded on any machine** — faster than DGX Spark's GPU GS (33.24s) and every CPU machine (`genoa` 22.25s, Rikyu CPU 26.66s, Mac 94.4s). But per this skill's own methodology, **GS is prep, not the benchmark** — the actual figure of merit (TDDFT `nt=300` per-step cost) is still an open gap for Rikyu's GPU build. Don't cite the GS number alone as "Rikyu GPU is fastest" without the TDDFT result; DGX Spark's pattern (GS unremarkable, TDDFT 4.6x faster than CPU) suggests GS time is a poor predictor of TDDFT time across GPU machines. |
