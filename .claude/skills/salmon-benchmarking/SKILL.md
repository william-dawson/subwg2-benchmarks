...
| R-CCS Cloud `genoa`, **hybrid** (4 ranks × 24 threads = 96 cores) | Same build, `mpirun -n 4 --bind-to none`, `OMP_NUM_THREADS=24` (salmon-build recipe) | Same 1×1×1 input | 11.997s (same convergence as pure-MPI, 1.86x faster than `OMP_NUM_THREADS=1`) | **14.82s for `nt=300`** (electron count/energy identical to pure-MPI TDDFT) | Hybrid wins on both stages: 1.86x (GS) and **3.3x** (TDDFT, 49.48s→14.82s) faster than pure-MPI at the same 96-core budget. `--bind-to none` is safe here specifically because `genoa` is single-socket (see `salmon-build`) — don't reuse this exact flag on a multi-socket machine. |
| R-CCS Cloud `fx700` (Fujitsu A64FX) | GCC + MPICH toolchain (**not** the Fujitsu compiler — see `salmon-build` for why the Fujitsu-compiler+Open-MPI build reliably crashes on multi-rank execution), `mpirun -n 4` | Same 1×1×1 input | not separately timed in this run (reused an existing restart from earlier `fx700` GS work this session) | **190.77s for `nt=300`** (internal timer; electron count conserved to ~1e-7, final energy -6099.81098920 eV — exact match to every other machine) | By far the **slowest TDDFT time recorded on any machine** (19x slower than `genoa` pure-MPI's 49.48s, 66x slower than Rikyu GPU's 2.86s) — expected given A64FX's per-core performance and that this used the fallback GCC+MPICH toolchain rather than Fujitsu's own (broken for multi-rank, see `salmon-build`) compiler. Correctness confirmed regardless; this closes out the last pending `fx700` item from earlier in the session (job queued a long time due to full partition occupancy, eventually completed cleanly). |
...
| RIKYU, **hybrid** (4 ranks × 36 threads = 144 cores) | Same build, `mpirun -n 4 --map-by socket:PE=36 --bind-to core`, `OMP_NUM_THREADS=36`, `cpu_cores_per_process=36` + `gpus=4` in the Slurm resource spec (Rikyu ties CPU allocation to GPU request count — see salmon-build) | Same 1×1×1 input, restart reused directly from the pure-MPI 4-rank GS run (rank count for restart doesn't matter, only the physical system) | n/a (reused existing converged restart, GS not rerun) | **3.11s for `nt=300`** (electron count/energy identical to pure-MPI TDDFT) | **11x faster** than pure-MPI's 34.35s at the same 144-core budget — the single best hybrid TDDFT speedup recorded so far across any machine, notably bigger than `genoa`'s 3.3x at a comparable per-rank thread count (24 vs 36). Worth a deeper look if Rikyu becomes a primary benchmarking target. |
| RIKYU GPU (GB200/Blackwell, `nvhpc-openacc` build, `nvhpc/26.3`), **1 rank = 1 GPU** | NVIDIA HPC-X MPI (bundled with `nvhpc/26.3` module), `mpirun -n 1` (salmon-build recipe) | Same 1×1×1 input | **16.82s** (internal `total calculation time` from the code's own timer, not wall-clock — 100/100 `nscf` iterations, converged: Total Energy -6099.94333354 eV, gap 5.717 eV — correct, matches every other machine) | **2.86s for `nt=300`** (internal timer; energy conserved, -6099.81098920 eV final — exact match to every other machine's TDDFT) | GS is the **fastest GS time recorded on any machine** — faster than DGX Spark's GPU GS (33.24s) and every CPU machine. TDDFT is also the fastest recorded — faster than DGX Spark's 10.81s. Both now measured, so this is a genuine best-in-class single-GPU result, not just a GS artifact (see the earlier note in this row's history about not trusting GS alone as a predictor — that caveat is now resolved for this machine). |
| RIKYU GPU, **2 ranks = 2 GPUs**, single node | Same build + `nvhpc/26.3`, `mpirun -n 2 ./wrapper.sh "$BIN"` (per-rank `CUDA_VISIBLE_DEVICES` binding, manual §2.6.5), built with `-DFORTRAN_COMPILER_HAS_MPI_VERSION3=OFF` (works around a real SALMON/MPI hang — see salmon-build), TDDFT additionally needs explicit `&parallel/nproc_k=1 nproc_ob=2 nproc_rgrid=1,1,1` (works around a separate restart-redistribution bug) | Same 1×1×1 input | 33.4s (100/100 `nscf`, correct: -6099.94333354 eV) | **4.41s for `nt=300`** (internal timer; correct: -6099.81098920 eV final) | **Slower than 1 GPU on both stages** (GS: 33.4s vs 16.82s; TDDFT: 4.41s vs 2.86s, ≈1.5x slower) — this 18-atom/96-electron system is too small to amortize 2-GPU communication overhead. Both fixes above required extensive debugging (see salmon-build's Rikyu GPU section) — a hang and a silent correctness bug were both found and fixed here, not just a scaling-inefficiency story. |
| RIKYU GPU, **4 ranks = 4 GPUs**, single node | Same build/fixes as 2-GPU row, `mpirun -n 4 ./wrapper.sh`, TDDFT `nproc_ob=4` | Same 1×1×1 input | 25.2s (100/100 `nscf`, correct: -6099.94333354 eV) | **6.29s for `nt=300`** (internal timer; correct: -6099.81098920 eV final) | Still slower than 1 GPU, and slower than 2 GPUs too (TDDFT keeps getting worse: 2.86s → 4.41s → 6.29s as GPU count increases) — confirms this problem size is fundamentally too small for multi-GPU to pay off, not a fluke specific to 2 GPUs. Resolved by the 2×2×2 sweep below: a bigger supercell does show real multi-GPU speedup. 8 GPUs (2 nodes) hits a separate, unresolved hang — not yet benchmarked. |
| RIKYU GPU, **1 rank = 1 GPU**, **2×2×2 supercell** | Same build/`nvhpc/26.3`, `mpirun -n 1` | `benchgen salmon create --nx 2 --ny 2 --nz 2 --nt 300`: 144 atoms, 768 electrons, 576 states, 57×98×62 grid | **550.27s** (100/100 `nscf`, correct: Total Energy -48808.13756839 eV, gap 5.71643146 eV — same gap as the 1×1×1 cell, as expected for a bulk supercell) | **46.18s for `nt=300`** (internal timer; correct final energy -48807.0796871967 eV) | First benchmark at a size large enough to test whether multi-GPU scaling is real or just an artifact of the tiny 18-atom cell — see the 2/4-GPU rows immediately below for the answer. |
| RIKYU GPU, **2 ranks = 2 GPUs**, **2×2×2 supercell** | Same build/fixes as the 1×1×1 2-GPU row, but **also needs explicit `nproc_ob=2` for GS this time** (not just TDDFT) — the auto-selected default for this bigger grid picked a real-space split (`nproc_rgrid=1,1,2`), which was measurably *slower* than 1 GPU (ran at ~17s/SCF-iteration vs. 1-GPU's ~5.5s/iteration, killed before completion); switching to explicit `&parallel/nproc_k=1 nproc_ob=2 nproc_rgrid=1,1,1` (orbital parallelization, no spatial split, so no halo-exchange communication) fixed it | Same 2×2×2 input | **454.03s** (correct: -48808.13756840 eV, matches 1-GPU to 8 decimals) — **1.21x speedup** | **36.93s** (correct: -48807.0796871967 eV, matches 1-GPU to ~10 sig figs) — **1.25x speedup** | **First genuine multi-GPU speedup recorded for SALMON on Rikyu**, both stages. Confirms the 1×1×1 slowdown really was a too-small-problem artifact, not a fundamental limit of multi-GPU on this machine. `nproc_rgrid` (real-space split) is measurably worse than `nproc_ob` (orbital split) for this system/grid size — default to `nproc_ob` for multi-GPU SALMON on Rikyu going forward, don't trust the auto-selected default. |
| RIKYU GPU, **4 ranks = 4 GPUs**, **2×2×2 supercell** | Same as 2-GPU row, `nproc_ob=4` | Same 2×2×2 input | **534.64s** (correct: -48808.13756840 eV) — **1.03x speedup, plateaued/regressed from 2-GPU** | **31.97s** (correct: -48807.0796871967 eV) — **1.44x speedup, kept improving** | **GS and TDDFT scale differently at 4 GPUs**: GS peaks at 2 GPUs and regresses toward 1-GPU parity at 4 (SCF's mixing/eigensolver steps plausibly have a higher communication-to-compute ratio that starts to dominate past 2 ranks), while TDDFT keeps improving all the way to 4 GPUs. Reinforces this skill's "GS is prep, not the benchmark" framing — the number that actually matters (TDDFT) scales the best. 8 GPUs (2 nodes) not yet attempted for this size — the 2-node hang from the 1×1×1 benchmarking (see salmon-build) would need resolving first. |
